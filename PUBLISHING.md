...
本项目通过公共仓库 [elfmedy/branch-note](https://github.com/elfmedy/branch-note) 的 GitHub Releases 提供安装包，支持 Obsidian 社区目录和 BRAT。当前版本 **0.1.4**。
...
4. 标签严格使用版本号（例如 `0.1.4`），不加 `v`。推送版本标签后，GitHub Actions 必须检查通过，才会创建 Release。
...
```sh
gh release download 0.1.4 --repo elfmedy/branch-note
